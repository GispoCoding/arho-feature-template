...
from qgis.PyQt.QtGui import QIcon, QPixmap, QMouseEvent
from qgis.PyQt.QtWidgets import QHBoxLayout, QLabel, QPushButton, QSizePolicy, QSplitter, QToolButton, QWidget
...
class RegulationGroupWidget(QWidget, FormClass):  # type: ignore
    ...
    def add_regulation(self, regulation: Regulation):
        if USE_BUTTONS:
            widg = QPushButton(text=f"– Kaavaamääräys: {regulation.config.name}", parent=self.frame)
            widg.setStyleSheet("""
                QPushButton {
            ...
            widg = QLabel(text=f"– Kaavaamääräys: {regulation.config.name}", parent=self.frame)
